...
* Retrofit2 - 인터파크 API를 활용해 서버에 저장된 데이터를 받아오기 위해 Retrofit2 라이브러리를 활용하였습니다. Data class를 정의하여 데이터 모델을 만들고 DTO(Data Transfer Object)를 만들어 JSON 타입을 변환해서 key 값을 직렬화하여 지정한 property에 담기도록 하였습니다. 검색 결과를 가져오고 베스트셀러 목록을 가져오는 통신을 정의하는 서비스 interface를 정의했습니다.
